...
Severity rendering: 1 -> `no`, 2 -> `[!]`, 3 -> `[i]` (Information DOES render); the
...
- Lint-as-diagnostics: **shipped**. One lint-profile compile (same policies as
  ...
  LINT/PERF -> Warning, STYLE -> Information (probe-verified `[!]`/`[i]` render). Repo
...
[key] **Coordinate convention (probed + lexer-verified)**: daslang `LineInfo`
