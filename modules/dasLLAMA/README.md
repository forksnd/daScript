...
- **Text LLMs** (GGUF, arch auto-detected): Llama-2 / Llama-3 (incl. TinyLlama, SmolLM2,
  Mistral-7B), Mistral-Medium-3.5-128B, Qwen2.5, Qwen3, Qwen-MoE (Qwen1.5-MoE, Qwen3-30B-A3B),
  the Qwen3.5 / Qwen3.6 hybrid-DeltaNet family — dense 0.8B–27B and MoE 35B-A3B / 122B-A10B,
  including the `-MTP-` files' NextN head for **self-speculative decode** (~1.4× decode on the
  27B, output-invariant) — Qwen3-Coder-Next (80B-A3B), GLM-4.5-Air (106B-A12B MoE), Phi-3.5, Gemma-2, Gemma-3, Gemma-4 (dense
  12B/31B, 26B-A4B MoE, and the E2B/E4B edge models with per-layer embeddings + cross-layer KV
  sharing), gpt-oss-20b.
...
demos are in `examples/dasLLAMA/` and tests in `modules/dasLLAMA/tests/`.
...
| **Mistral-Medium-3.5-128B** | Q4_K_M GGUF, 3-shard split (native K-quant planes) | Mistral3 (arch `mistral3` — the llama dense shape at 88 layers / embd 12288 / 96Q 8KV heads, NORM rope) with **YaRN** rope (factor 64 over a 4096 original context, **beta_fast 4 / beta_slow 1 from metadata** — the correction band and the 1.416 magnitude correction now key-driven; this model exposed the NORM-rope mscale drop, fixed in the same change). `attention.temperature_scale` (llama4-style Q-scaling) and `yarn_log_multiplier` ship as 0 and are fail-closed panics if nonzero | BPE (`pixtral` pre = the llama3 splitter; verified id-for-id vs the oracle) | ✅ | `llama.cpp` `simple_ids`: 40/40 token-for-token on the counting continuation (frozen huge-tier fixture in `test_parity.das`); chat via the detected `[SYSTEM_PROMPT]`/v7-tekken template |
...
| **GLM-4.5-Air (106B-A12B)** | Q4_K_M GGUF 2-shard + Q8_0 GGUF 3-shard splits | GLM-4.5-MoE (arch `glm4moe`: **1 leading dense layer** + 45 routed-MoE layers — 128 experts top-8 over a **sigmoid router with the DeepSeek-style `exp_probs_b` selection bias** (biased pick, unbiased renormalized mixing weights) + an **ungated shared expert** (no `ffn_gate_inp_shexp`, added at weight 1); QKV bias, 96Q/8KV heads of 128, **partial NEOX rotary 64/128 on the standard attention path**, pre-FFN norm named `post_attention_norm`; the trailing NextN block ships its OWN `embed_tokens`/`shared_head_head`, so the loader disables the MTP draft head with a warning — trunk = 46 layers, exactly llama.cpp's forward) | BPE (`glm4` pre = the llama3 splitter; verified id-for-id vs the oracle, no BOS — glm4 nulls it) | ✅ | `llama.cpp` `simple_ids`: 40/40 token-for-token on the counting continuation, both quants (Q8_0's own oracle continues identically; frozen huge-tier fixtures in `test_parity.das`); chat via the detected `[gMASK]<sop>` template (prelude + role markers, thinking on by default) |
...
| **Gemma-2-2B-it** | Q8_0 GGUF | Gemma2 (GeGLU, dual softcap, sliding window, sandwich norms, embed ×√dim) | SPM (GGUF) | ✅ | `llama.cpp` `simple_ids`, token-for-token; frozen fixture in `modules/dasLLAMA/tests/test_parity.das`; SWA exercised on a 4k+ context |
...
| Attention biases | **Qwen2** / **glm4moe** — Q/K/V projection biases; **gpt-oss** — Q/K/V + output-projection biases |
...
| Self-speculative decode (MTP/NextN) | qwen35-family `-MTP-` GGUFs load natively (`nextn_predict_layers == 1`): the in-file NextN draft block — one trunk-shaped attention layer at index `n_layers` plus `nextn.eh_proj/enorm/hnorm/shared_head_norm` — drafts one token ahead and a 2-row batched trunk forward verifies it, so accepted steps stream the weights once for two tokens. **Output-invariant vs plain greedy** (gated by `modules/dasLLAMA/tests/test_mtp.das` + the scheduler arm in `test_llm_scheduler.das`); 1.39× decode on Qwen3.6-27B Q8 flat-loop, ~2× through the server (zen2, list continuation). Engine gate `set_mtp_spec` + per-step `mtp_spec_eval`: prefills maintain the head's KV incrementally (chunked prefill included, seam rows via the `mtp_h` handoff), flat AND paged sessions (the pool carries the draft-block slab), every KV codec; `dasllama-server --mtp` spec-ticks greedy streams. Greedy-only v1; the DeltaNet recurrent state snapshots and rolls back around rejected drafts; `mtp_only` sidecar files panic honestly; a draft block with its OWN `nextn.embed_tokens`/`shared_head_head` (glm4moe) disables the MTP head with a warning — the das head drafts through the trunk's embedding/classifier, so it would use the wrong weights there (trunk forward unaffected; llama.cpp never runs the NextN layer either) |
...
3. Feed the same prompt IDs through dasLLAMA's forward pass (`harness/parity.das`) and diff the
   ...
   Once verified, freeze the prompt + continuation as a fixture in `modules/dasLLAMA/tests/test_parity.das`
